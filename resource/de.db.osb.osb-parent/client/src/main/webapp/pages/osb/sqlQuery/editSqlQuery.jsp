		...
		<jsp:include page="../../main_menu.jsp"/>
			...
			<html:form action="/saveSqlQuery" focus="name">
				...
				<div class="textcontent">
					<html:hidden styleId="sqlQueryId" property="sqlQueryId" />
					...
					<div class="textcontent_right">						
						<div class="box">
							...
							<div class="input">
								<html:text property="gueltigVon" styleId="gueltigVon" maxlength="10" styleClass="date" errorStyle="${errorStyle}" />
						...
						<div class="box">
							...
							<div class="input">
								<html:text property="gueltigBis" styleId="gueltigBis" maxlength="10" styleClass="date" errorStyle="${errorStyle}" />
						...
						<div class="box">
							...
							<div class="input">
								<html:select name="sqlQueryForm" property="modul" styleId="modul" disabled="true">
									<logic:iterate id="current" name="modulListe">
										<html:option value="${current}">${current}</html:option>
									</logic:iterate>
								</html:select>
								<!-- select auf enabled und hidden wegnehmen -->
								<html:hidden name="sqlQueryForm" property="modul" value="OSB" />
							</div>
						</div>
						<div class="box">
				...
				<div class="buttonBar">
					...
					<logic:present name="sqlQuery">
						<html:link action="/executeSqlQuery.do?sqlQueryId=${sqlQuery.id}" target="_blank" styleClass="buttonSearch"><bean:message key="osb.sqlQuery.execute" /></html:link>
						<bean:define id="confirmText" toScope="page"><bean:message key="confirm.sqlQuery.delete" /></bean:define>
						<html:link action="/deleteSqlQuery" paramId="sqlQueryId" paramName="sqlQuery" paramProperty="id" styleClass="buttonDelete" onclick="return confirmLink(this.href, '${confirmText}');"><bean:message key="button.delete" /></html:link>
					</logic:present>					
