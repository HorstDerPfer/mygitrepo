...
public class FahrplanregelungAddToBuendelAction extends BaseAction {

	private static final Logger log = Logger.getLogger(FahrplanregelungAddToBuendelAction.class);

	@Override
	protected ActionForward run(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	    HttpServletResponse response) throws Exception {
		if (log.isDebugEnabled())
			log.debug("Entering FahrplanregelungAddToBuendelAction.");
		...
		if (EasyServiceFactory.getInstance().createBuendelAnyVoter().vote(secUser, buendel,
		    "ROLE_BUENDEL_FAHRPLANREGELUNG_ANLEGEN") == AccessDecisionVoter.ACCESS_DENIED) {
			if (log.isDebugEnabled())
		...
		frService.attachBuendel(fahrplanregelung, buendel);
