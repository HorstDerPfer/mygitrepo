 ...
 * 
 */
public class PaketAnyVoter implements EasyAccessDecisionVoter {

	private Logger log = Logger.getLogger(PaketAnyVoter.class);
	...
	public int vote(final TqmUser tqmUser, Object domainObject, String requestedAuthorizations) {
		Authentication authentication = new Authentication() {
			...
			private static final long serialVersionUID = 742607169922421224L;
					...
					if (authority.getAuthority().startsWith(configAttribute.getAttribute())) {
						...
						if (authority.getAuthority().endsWith("_ALLE")) {
							...
							if (user.getRegionalbereich() != null
							    && paket.getRegionalbereich() != null
							    && paket.getRegionalbereich().equals(user.getRegionalbereich())) {
								result = ACCESS_GRANTED;
							} else {
