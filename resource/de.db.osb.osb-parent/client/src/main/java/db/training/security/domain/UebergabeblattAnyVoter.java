 ...
 */
public class UebergabeblattAnyVoter implements EasyAccessDecisionVoter {

	Logger logger = Logger.getLogger(UebergabeblattAnyVoter.class);
	...
	public int vote(Authentication authentication, Object object, ConfigAttributeDefinition config) {
		...
		Uebergabeblatt ueb = (Uebergabeblatt) object;
		String masterRB = "";
		try {
			masterRB = ueb.getMassnahmen().iterator().next().getMasterniederlassung().substring(3);
				...
				for (int i = 0; i < authorities.length; i++) {
					if (authorities[i].getAuthority().startsWith(attribute.getAttribute())) {
						...
						if (attribute.getAttribute().startsWith("ROLE_BBZR_ANLEGEN")) {
							// Anlegen unabhaengig von Regionen erlauben
						...
						if (authorities[i].getAuthority().endsWith("_ALLE")) {
							...
							if (user.getRegionalbereich().getName().equalsIgnoreCase(masterRB)) {
								result = ACCESS_GRANTED;
							} else if (attribute.getAttribute().startsWith("ROLE_BBZR_BEARBEITEN")) {
								// wenn ZVF, dann keine Prüfung beteiligte RB
								if (ueb.getMassnahmen().iterator().next().getVersion()
								    .getFormular().toString().startsWith("ZVF")) {
									result = ACCESS_GRANTED;
								} else {
									// wenn RB an UEB beteiligt ist, dann darf er auch bearbeiten
									Set<Regionalbereich> beteiligteRB = ueb.getBeteiligteRb();
									if (beteiligteRB != null) {
										for (Regionalbereich rb : beteiligteRB) {
											if (rb.equals(user.getRegionalbereich())) {
												result = ACCESS_GRANTED;
											}
										}
									}
